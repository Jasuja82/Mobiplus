...
export async function GET(request: NextRequest) {
  try {
    ...
    // Apply filters
    if (location !== "all") {
      vehicleQuery = vehicleQuery.eq("home_location_id", location)
      refuelQuery = refuelQuery.eq("fuel_station.location_id", location)
    }
    ...
    // Execute queries
    const [vehiclesData, refuelData] = await Promise.all([vehicleQuery, refuelQuery])

    // Calculate analytics
    const totalCost = refuelData.data?.reduce((sum, r) => sum + r.total_cost, 0) || 0
    const totalFuel = refuelData.data?.reduce((sum, r) => sum + r.liters, 0) || 0
    const totalKilometers = refuelData.data?.reduce((sum, r) => sum + (r.odometer_difference || 0), 0) || 0
    ...
    const [prevRefuelData] = await Promise.all([
      supabase
        .from("refuel_records")
    ...
    const prevTotalCost = prevRefuelData.data?.reduce((sum, r) => sum + r.total_cost, 0) || 0
    ...
    return NextResponse.json({
      ...
      analytics: {
        totalCost,
        totalFuel,
        totalMaintenance: 0, // Will be implemented when maintenance tables are recreated
        totalKilometers,
